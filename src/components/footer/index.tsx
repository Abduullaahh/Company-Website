import { tw } from 'twind';

const productLinks = [`Features`, `Customers`, `Platform`, `Pricing`, `Enterprise`, `What's new?`];
const aboutLinks = [`About Us`, `Careers`, `Leadership`, `Blog`, `Events`, `Press`];
const resourceLinks = [
  `Get started`,
  `Guides`,
  `Tools`,
  `Case studies`,
  `Solutions`,
  `FAQs`,
  `Help Center`,
  `Training`,
  `Other resources`,
];

const Footer = () => (
  <footer className={tw(`border-t border-gray-400 pt-14 pb-16`)} style={{ backgroundColor: `#014C8B` }}>
    <div className={tw(`max-w-7xl mx-auto text-gray-400 px-8 lg:px-0 flex flex-wrap`)}>
      <div className={tw(`mb-14 flex items-center w-full`)}>
        <img
          className={tw(`mr-4`)}
          src="/logof.svg"
          alt="DevSol - Expert Web Development & Digital Solutions"
          style={{ height: `5rem`, width: `9.375rem` }}
        />
      </div>
      <div className={tw(`w-full lg:w-1/2 text-white`)}>
        <ul className={tw(`text-lg font-light flex flex-wrap w-full`)}>
          <li className={tw(`w-1/2 md:w-1/3 lg:w-1/3`)}>
            <div>
              <h4 className={tw(`text-base font-bold mb-1`)}>Product</h4>
              <ul>
                {productLinks.map((link) => (
                  <li className={tw(`text-sm font-medium leading-6`)} key={link}>
                    <a href="/">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
          </li>
          <li className={tw(`w-1/2 md:w-1/3 lg:w-1/3`)}>
            <div>
              <h4 className={tw(`text-base font-bold mb-1`)}>Resources</h4>
              <ul>
                {resourceLinks.map((link) => (
                  <li className={tw(`text-sm font-medium leading-6`)} key={link}>
                    <a href="/">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
          </li>
          <li className={tw(`w-1/2 md:w-1/3 lg:w-1/3`)}>
            <div>
              <h4 className={tw(`text-base font-bold mb-1`)}>About Us</h4>
              <ul>
                {aboutLinks.map((link) => (
                  <li className={tw(`text-sm font-medium leading-6`)} key={link}>
                    <a href="/">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
          </li>
        </ul>
      </div>
      <div className={tw(`w-full lg:w-1/2 mt-12 lg:mt-0`)}>
        <div className={tw(`py-5 px-4`)}>
          <h4 className={tw(`font-mono text-sm uppercase text-white mb-3`)}>Subscribe our newsletter</h4>
          <div className={tw(`flex w-full`)}>
            <input
              aria-label="email address"
              type="text"
              className={tw(`border border-gray-300 bg-gray-100 min-w-0 w-full rounded py-2 px-3 mr-2`)}
              placeholder="Enter your email"
            />
            <button
              className={tw`px-6 py-2 text-white rounded-md transition duration-300`}
              onMouseEnter={(e) => (
                (e.currentTarget.style.backgroundColor = `white`), (e.currentTarget.style.color = `#003B6A`)
              )}
              onMouseLeave={(e) => (
                (e.currentTarget.style.backgroundColor = `#003B6A`), (e.currentTarget.style.color = `white`)
              )}
              style={{ backgroundColor: `#003B6A` }}
            >
              Subscribe
            </button>
          </div>
        </div>
      </div>
    </div>
  </footer>
);

export default Footer;
