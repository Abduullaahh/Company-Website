...
const ProjectsSection = () => (
  <section>
    <div className={tw(`w-full min-h-screen relative`)} style={{ backgroundImage: 'linear-gradient(to right, #014783, #0287DF)' }} aria-label="Projects Overview" id="projects">
        ...
        <div className={tw(`mx-auto pt-24`)}>
          <div className={tw(`w-full flex flex-wrap justify-around`)}>
            {projects.map((project) => (
              <Link
                key={project.slug}
                className={tw(`xl:w-1/3 sm:w-5/12 sm:max-w-xs relative mb-32 lg:mb-20 xl:max-w-sm lg:w-1/2 w-11/12 mx-auto sm:mx-0 cursor-pointer transition-transform duration-300 ease-in-out hover:scale-105`)}
                href={`/project-description/${project.slug}?data=${encodeURIComponent(JSON.stringify(project))}`}
                passHref
              >
                <div>
            ...
            <span
              className={tw(`-mt-8 pb-12 lg:mt-4 flex items-center text-xl text-white cursor-pointer z-30 transition-all duration-300 hover:mr-6 hover:ml-2`)}
            >
              See all projects
              <Arrow className={tw(`h-6 w-6 fill-current ml-2 transition-transform duration-300 transform hover:translate-x-2`)} />
            </span>
          </div>
        </div>
      </div>
